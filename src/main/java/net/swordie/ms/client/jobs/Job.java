...
public abstract class Job {
	...
	public void handleAttack(Char chr, AttackInfo attackInfo) {
		...
		for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
			...
			if (attackInfo.skillId != Bishop.BAHAMUT && mts.hasStat(MobStat.BahamutLightElemAddDam)) {
	...
	public int getMpCon(int skillId, int slv) {
		...
		mpCon += (int) (mpCon * ((double) chr.getTotalStat(BaseStat.costMpR) / 100D));
		mpCon -= (int) (mpCon * ((double) chr.getTotalStat(BaseStat.mpConReduce) / 100D));
		return mpCon;
	}
